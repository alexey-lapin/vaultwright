...
#   build/stubs/<role>/<os>_<arch>.stub
#   build/SHA256SUMS   (sha256sum format: "<hash>  stubs/<role>/<os>_<arch>.stub")
#
...
VAULT_TARGETS=${VAULT_TARGETS:-"darwin/arm64 darwin/amd64 linux/amd64 linux/arm64 windows/amd64 windows/arm64"}
WARDEN_TARGETS=${WARDEN_TARGETS:-"darwin/arm64 darwin/amd64 linux/amd64 linux/arm64 windows/amd64 windows/arm64"}
OUT=${OUT:-build}
LDFLAGS=${LDFLAGS:-"-s -w"}
...
# Manifest with paths relative to the output dir so verification is location-independent.
